...
const formatSizes = (sizes: Record<string, number>) => {
  ...
  return sortedSizes.map(([size, qty]) => `${size}: ${qty}`).join(', ')
}
...
export default function GradesDataTable({
  ...
  const columns = React.useMemo<ColumnDef<Grade>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-center">
          <span className="font-mono text-sm text-gray-600">{row.getValue('slug')}</span>
        </div>
      ),
    },
    {
      ...
      header: 'Tamanhos',
      cell: ({ row }) => {
        const sizes = row.getValue('sizes') as Record<string, number>
        return (
          <div className="text-center">
            <div className="text-sm text-gray-600 truncate max-w-xs" title={formatSizes(sizes)}>
              {formatSizes(sizes)}
            </div>
          </div>
        )
    ...
    },
    {
      id: 'total',
      header: 'Total',
      accessorFn: (row) => getTotalPairs(row.sizes),
      cell: ({ row }) => {
        const sizes = row.original.sizes
        return (
          <div className="text-center">
            <Badge variant="outline">
              {getTotalPairs(sizes)} pares
            </Badge>
          </div>
        )
      },
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Tem certeza que deseja excluir a grade "{grade.name}"? 
    ...
    <div className="w-full space-y-3">
      ...
      <div className="flex items-center justify-between py-3 px-1">
        <div className="flex-1 text-sm text-gray-600">
          {table.getFilteredRowModel().rows.length} grade(s) encontrada(s).
        </div>
