...
interface GendersDataTableProps {
  genders: Gender[]
...
export default function GendersDataTable({
  genders,
  onEdit,
  onDelete,
}: GendersDataTableProps) {
  const [sorting, setSorting] = React.useState<SortingState>([])
  const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>([])

  const columns = React.useMemo<ColumnDef<Gender>[]>(() => [
    {
      accessorKey: 'id',
      header: ({ column }) => {
        return (
    ...
    <div className="w-full space-y-3">
      ...
      <div className="flex items-center justify-between py-3 px-1">
        <div className="flex-1 text-sm text-gray-600">
          {table.getFilteredRowModel().rows.length} gênero(s) encontrado(s).
        </div>
